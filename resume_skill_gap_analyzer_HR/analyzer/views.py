...
def analyze_resume(request, resume_id, job_id):
    ...
    if created or not analysis.matched_skills:
        try:
            ...
            analysis.partially_matched_skills = format_skills_list(match_result.get('partially_matched_skills', []))
            ...
            messages.success(request, f'Resume analysis completed! Match Score: {match_result["match_score"]}%')
...
def analysis_result(request, analysis_id):
    ...
    partially_matched_skills = parse_skills_from_string(analysis.partially_matched_skills) if hasattr(analysis, 'partially_matched_skills') else []
            ...
            for file in files:
                try:
                    ...
                    SkillAnalysis.objects.create(
                        ...
                        partially_matched_skills=format_skills_list(match_result.get('partially_matched_skills', [])),
...
def all_resumes(request):
    """View all uploaded resumes in a table format (HR only)"""
    try:
        profile = request.user.userprofile
        if profile.role != 'HR':
            messages.error(request, 'Access denied.')
    ...
    # Get all resumes with related user information
    ...
    # Get all jobs for quick analysis
    jobs = Job.objects.filter(hr=request.user)
    ...
    return render(request, 'hr/all_resumes.html', context)


@login_required
def quick_analyze(request, resume_id):
    """Quick analyze a resume against a job (HR only)"""
    try:
        profile = request.user.userprofile
        if profile.role != 'HR':
            messages.error(request, 'Access denied.')
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages.success(request, f'Analysis completed! Match Score: {match_result["match_score"]}%')
            return redirect('candidate_detail', analysis_id=analysis.id)
